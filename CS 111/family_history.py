...
def main():
    ...
    count_marriages(marriages_dict, people_dict, counter_dict)
...
def print_death_age(people_dict):
    ...
    for key in people_dict:
        ...
        age = int(person[DEATH_YEAR_INDEX]) - int(person[BIRTH_YEAR_INDEX])
        print(f"{person[NAME_INDEX]}, age: {age}, birth year: {person[BIRTH_YEAR_INDEX]}, year of death: {person[DEATH_YEAR_INDEX]}")
    pass
...
def count_genders(people_dict):
    ...
    for key in people_dict:
        ...
        if person[GENDER_INDEX] == "F":
            female += 1
        elif person[GENDER_INDEX] == "M":
            male += 1

    print(f"There are {female} females and {male} males in the data set.")
    pass

def count_marriages(marriages_dict, people_dict, counter_dict):
    for key in marriages_dict:
        ...
        if marriage[HUSBAND_KEY_INDEX] not in counter_dict:
            counter_dict[marriage[HUSBAND_KEY_INDEX]] = [husband[NAME_INDEX], 1]
        else:
            list = counter_dict[marriage[HUSBAND_KEY_INDEX]] 
            list[1] += 1
            counter_dict[marriage[HUSBAND_KEY_INDEX]] = list
        if marriage[WIFE_KEY_INDEX] not in counter_dict:
            counter_dict[marriage[WIFE_KEY_INDEX]] = [wife[NAME_INDEX], 1]
        else:
            list = counter_dict[marriage[WIFE_KEY_INDEX]] 
            list[1] += 1
            counter_dict[marriage[WIFE_KEY_INDEX]] = list
    for key in counter_dict:
        person = counter_dict[key]
        print(f"{person[NAME_INDEX]} was married {person[1]} time(s)")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for key in marriages_dict:
        ...
        husband = people_dict[marriage[HUSBAND_KEY_INDEX]]
        wife = people_dict[marriage[WIFE_KEY_INDEX]]
        wife_age = int(marriage[WEDDING_YEAR_INDEX]) - int(wife[BIRTH_YEAR_INDEX])
        husband_age = int(marriage[WEDDING_YEAR_INDEX]) - int(husband[BIRTH_YEAR_INDEX])
        print(f"{husband[NAME_INDEX]} {husband_age} - {marriage[WEDDING_YEAR_INDEX]} - {wife[NAME_INDEX]} {wife_age}")
